...
use super::{
    binder::Binder,
    const_ty::{CallableInputHoleCx, ConstTyId},
    fold::{TyFoldable, TyFolder},
    trait_def::{ImplementorId, ImplementorOrigin, TraitInstId, does_impl_trait_conflict},
    trait_resolution::PredicateListId,
    ty_def::{InvalidCause, TyId},
    ty_lower::{
        ConstDefaultCompletion, lower_hir_ty, lower_hir_ty_with_callable_holes, lower_opt_hir_ty,
        lower_opt_hir_ty_with_callable_holes,
    },
};
use crate::analysis::{
    HirAnalysisDb,
...
pub(crate) fn lower_trait_ref<'db>(
    ...
    lower_trait_ref_inner(db, self_ty, trait_ref, scope, assumptions, owner_self, None)
}

pub(crate) fn lower_trait_ref_with_callable_holes<'db>(
...
pub(crate) fn lower_trait_ref_impl<'db>(
    ...
    lower_trait_ref_impl_inner(db, path, scope, assumptions, t, None)
}

pub(crate) fn lower_trait_ref_impl_with_callable_holes<'db>(
    db: &'db (dyn HirAnalysisDb + 'static),
    path: PathId<'db>,
    scope: ScopeId<'db>,
    assumptions: PredicateListId<'db>,
    ...
    lower_trait_ref_impl_inner(db, path, scope, assumptions, t, Some(callable_holes))
}
...
fn lower_trait_ref_impl_inner<'db>(
        ...
        match arg {
            GenericArg::Type(ty_arg) => {
                let ty = callable_holes
                    ...
                    .unwrap_or_else(|| lower_opt_hir_ty(db, ty_arg.ty, scope, assumptions));
            ...
            GenericArg::AssocType(AssocTypeGenericArg { name, ty }) => {
                if let (Some(name), Some(ty)) = (name.to_opt(), ty.to_opt()) {
                    let ty = callable_holes
                        .map(|callable_holes| {
                            lower_hir_ty_with_callable_holes(
                                db,
                                ty,
                                scope,
                                assumptions,
                        ...
                        .unwrap_or_else(|| lower_hir_ty(db, ty, scope, assumptions));
    ...
    let non_self_completed = t.param_set(db).complete_explicit_args(
        ...
        &provided_explicit,
        assumptions,
        ConstDefaultCompletion::evaluate(Some(path)).with_callable_holes(callable_holes),
    );
...
mod tests {
    ...
    use crate::analysis::ty::{
        const_ty::{ConstTyData, LayoutHoleArgSite, LayoutHoleId},
        trait_resolution::PredicateListId,
        ty_def::TyData,
    };
        ...
        assert!(matches!(
            ...
            ConstTyData::Hole(
                ...
                LayoutHoleId::ExplicitArg {
                    site: LayoutHoleArgSite::Path(hole_path),
                    arg_idx: 1,
                },
            ) if *hole_path == path
        ));
