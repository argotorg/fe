    ...
    for (expr, _) in body.exprs(db).iter() {
        ...
        for &predicate in predicates {
            ...
            if !failures.is_empty() {
                ...
                diags.extend(failures.iter().cloned());
            }
        }
    }
    diags
...
fn discharge_requirement<'db>(
    ...
    if symbolic {
        ...
        if predicate_may_depend_on_params(db, predicate) {
            return vec![BodyDiag::ConstRequirementNotSatisfied {
                primary: predicate.span().into(),
                predicate: predicate.span().into(),
                reason: if key.is_some() {
                    "no matching const requirement in the caller after substitution".into()
                } else {
                    "symbolic forwarding of this expression is not supported; concrete evaluation is required".into()
                },
            }.into()];
        }
    }
    ...
    match eval_body_owner_const(db, owner, args) {
        ...
        Err(error) => {
            let ty = TyId::invalid(db, invalid_cause_from_ctfe_error(db, owner, error));
            vec![
