...
impl<'db> SymbolInfo<'db> for TopLevelMod<'db> {
    fn display_name(&self) -> &'static str { "module" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Some(TopLevelMod::name(*self, db)) }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { None }
    fn top_mod(&self, _db: &'db dyn HirDb) -> TopLevelMod<'db> { *self }
}

impl<'db> SymbolInfo<'db> for Mod<'db> {
    fn display_name(&self) -> &'static str { "module" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Mod::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Mod::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Func<'db> {
    fn display_name(&self) -> &'static str { "function" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Func::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Func::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Struct<'db> {
    fn display_name(&self) -> &'static str { "struct" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Struct::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Struct::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Contract<'db> {
    fn display_name(&self) -> &'static str { "contract" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Contract::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Contract::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Enum<'db> {
    fn display_name(&self) -> &'static str { "enum" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Enum::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Enum::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for TypeAlias<'db> {
    fn display_name(&self) -> &'static str { "type alias" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { TypeAlias::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().alias().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { TypeAlias::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Impl<'db> {
    fn display_name(&self) -> &'static str { "impl" }
    fn name(&self, _db: &'db dyn HirDb) -> Option<IdentId<'db>> { None }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { None }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Impl::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Trait<'db> {
    fn display_name(&self) -> &'static str { "trait" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Trait::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Trait::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for ImplTrait<'db> {
    fn display_name(&self) -> &'static str { "impl trait" }
    fn name(&self, _db: &'db dyn HirDb) -> Option<IdentId<'db>> { None }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { None }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { ImplTrait::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Const<'db> {
    fn display_name(&self) -> &'static str { "constant" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> { Const::name(*self, db).to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Const::top_mod(*self, db) }
}

impl<'db> SymbolInfo<'db> for Use<'db> {
    fn display_name(&self) -> &'static str { "use" }
    fn name(&self, _db: &'db dyn HirDb) -> Option<IdentId<'db>> { None }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { None }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { Use::top_mod(*self, db) }
}

// View implementations

impl<'db> SymbolInfo<'db> for FieldView<'db> {
    fn display_name(&self) -> &'static str { "field" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> {
    ...
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { self.owner_item().top_mod(db) }
}

impl<'db> SymbolInfo<'db> for VariantView<'db> {
    fn display_name(&self) -> &'static str { "variant" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> {
        self.owner.variants_list(db).data(db)[self.idx].name.to_opt()
    }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { self.owner.top_mod(db) }
}
...
impl<'db> SymbolInfo<'db> for GenericParamView<'db> {
    ...
    fn name(&self, _db: &'db dyn HirDb) -> Option<IdentId<'db>> { self.name().to_opt() }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(GenericParamView::name_span(self).into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { ItemKind::from(self.owner).top_mod(db) }
}

impl<'db> SymbolInfo<'db> for FuncParamView<'db> {
    fn display_name(&self) -> &'static str { "parameter" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> {
        self.func.params(db).to_opt()?.data(db).get(self.idx)?.name()
    }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
    fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> { self.func.top_mod(db) }
}

impl<'db> SymbolInfo<'db> for TraitAssocTypeView<'db> {
    fn display_name(&self) -> &'static str { "associated type" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> {
...
impl<'db> SymbolInfo<'db> for TraitAssocConstView<'db> {
    fn display_name(&self) -> &'static str { "associated constant" }
    fn name(&self, db: &'db dyn HirDb) -> Option<IdentId<'db>> {
        self.decl(db).name.to_opt()
    }
    fn name_span(&self) -> Option<DynLazySpan<'db>> { Some(self.span().name().into()) }
