...
pub(super) fn lower_event_struct<'db>(
    ...
    let signature = build_event_signature(&struct_name_str, &parsed_fields.ordered_field_types);
    ...
    builder.with_item_scope(
        ...
        move |builder, id| {
            let topic0_const =
                create_topic0_const(builder.ctxt(), event_desugared.clone(), signature);
...
struct ParsedEventFields<'db> {
    ...
    ordered_field_types: Vec<EventFieldType>,
    indexed_fields: Vec<(IdentId<'db>, EventFieldType)>,
...
fn parse_event_fields<'db>(
    ...
    for field in fields {
        ...
        let field_ty = match parse_event_field_type(db, ty) {
            Some(ty) => ty,
            None => {
                EventError {
        ...
        ordered_field_types.push(field_ty);

        if is_indexed {
            indexed_fields.push((name_ident, field_ty));
        } else {
...
fn parse_event_field_type<'db>(
    db: &'db dyn crate::HirDb,
    ty: TypeId<'db>,
) -> Option<EventFieldType> {
    let TypeKind::Path(Partial::Present(path)) = ty.data(db) else {
        return None;
    };
    let ident = path.ident(db).to_opt()?;
    let name = ident.data(db);

    if name == "Address" {
        return Some(EventFieldType::Address);
    }
    if name == "bool" {
        return Some(EventFieldType::Bool);
    }

    let (signed, bits_str) = if let Some(rest) = name.strip_prefix('u') {
        (false, rest)
    ...
    Some(if signed {
        EventFieldType::Int { bits }
    } else {
        EventFieldType::Uint { bits }
    })
}

fn build_event_signature(event_name: &str, field_types: &[EventFieldType]) -> String {
    let mut out = String::new();
    out.push_str(event_name);
    out.push('(');
    for (idx, ty) in field_types.iter().copied().enumerate() {
        if idx != 0 {
            out.push(',');
        }
        out.push_str(&ty.abi_name());
    }
    out.push(')');
    out
}
...
fn create_topic0_const<'db>(
    ...
    let lit = Expr::Lit(LitKind::String(crate::hir_def::StringId::new(
        db, signature,
    )));
    let lit_id = body_ctxt.push_expr(lit, origin.clone());
    let call = Expr::Call(
        callee_id,
...
fn lower_emit_method<'db>(
    ...
    indexed_fields: &[(IdentId<'db>, EventFieldType)],
    ...
    let downcast_unchecked_ident = builder.ident("downcast_unchecked");
    ...
    builder.func_with_body(
        ...
        move |body| {
            ...
            for (name, ty) in indexed_fields.iter().copied() {
                let value = self_field_expr(body, self_expr, name);
                let topic = topic_expr(body, value, ty, roots.std, downcast_unchecked_ident);
                args.push(topic);
            }
...
fn topic_expr<'db>(
    ...
    value: crate::hir_def::ExprId,
    ty: EventFieldType,
    std_root: IdentId<'db>,
    downcast_unchecked_ident: IdentId<'db>,
) -> crate::hir_def::ExprId {
    match ty {
        EventFieldType::Address => {
            let db = body.db();
            let callee = body.path_expr(
                PathId::from_ident(db, std_root)
                    .push_str(db, "evm")
                    .push_str(db, "address_to_word"),
            );
            body.call_expr(callee, vec![value])
        }
        EventFieldType::Bool => {
            let then_block = block_expr_with_int(body, 1);
            let else_block = block_expr_with_int(body, 0);
            body.push_expr(Expr::If(value, then_block, Some(else_block)))
        }
        EventFieldType::Uint { .. } | EventFieldType::Int { .. } => {
            body.method_call_expr(value, downcast_unchecked_ident, vec![])
        }
    }
}
...
fn block_expr_with_int<'db>(
    body: &mut super::hir_builder::BodyBuilder<'_, 'db, EventDesugared>,
    v: usize,
) -> crate::hir_def::ExprId {
    let lit = int_lit(body, v);
    let stmt = body.push_stmt_raw(Stmt::Expr(lit));
    body.push_expr(Expr::Block(vec![stmt]))
}
