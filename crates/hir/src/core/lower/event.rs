...
/// Generate the expression that converts an indexed field value to a `u256` topic.
///
/// - `Address`  → `address_to_word(value)`
/// - `bool`     → `if value { 1 } else { 0 }`
/// - otherwise  → `value.downcast_unchecked()`
fn topic_expr<'db>(
    ...
    let db = body.db();
    let type_name = type_name_str(db, field_ty);
    ...
    match type_name.as_deref() {
        ...
        _ => body.method_call_expr(value, downcast_unchecked_ident, vec![]),
    }
}

/// Extract the simple type name from a `TypeId` (e.g. `"Address"`, `"u256"`, `"Uint160"`).
fn type_name_str<'db>(db: &'db dyn crate::HirDb, ty: TypeId<'db>) -> Option<String> {
    let TypeKind::Path(Partial::Present(path)) = ty.data(db) else {
        return None;
    };
    path.ident(db).to_opt().map(|id| id.data(db).to_string())
}
