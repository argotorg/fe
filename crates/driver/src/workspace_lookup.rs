...
fn ensure_remote_workspace_registry(
    ...
    let checkout = git
        .ensure_sparse_checkout(
            description,
            &[String::from("fe.toml")],
            resolver::git::SparseCheckoutMode::NoCone,
        )
...
fn select_workspace_member(
    ...
    description: Option<&GitDescription>,
) -> Result<WorkspaceMemberSelectionResult, WorkspaceLookupError> {
    ...
    if let Some(version) = version {
        ...
        for member in candidates.iter_mut() {
            if member.version.is_none()
                && member_version_matches(member, version, checkout_path, git, description)
                    .unwrap_or(false)
            {
                if matched.is_some() {
                    return Err(WorkspaceLookupError::Message(format!(
...
fn member_version_matches(
    ...
    let member_path = if let Some(checkout_root) = checkout_path {
        if let (Some(git), Some(description)) = (git, description) {
            let patterns = vec![format!("{}/fe.toml", member.path.as_str())];
            git.ensure_sparse_checkout(
                description,
                &patterns,
                resolver::git::SparseCheckoutMode::NoCone,
            )
