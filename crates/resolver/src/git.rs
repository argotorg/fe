...
pub struct GitResolver {
    ...
    diagnostics: Vec<GitResolutionDiagnostic>,
}
...
impl GitResolver {
    ...
    fn checkout_path(&self, description: &GitDependencyDescription) -> Utf8PathBuf {
    ...
    fn ingot_path(
        checkout_path: &Utf8Path,
        description: &GitDependencyDescription,
    ) -> Result<Utf8PathBuf, GitResolutionError> {
        if let Some(relative) = &description.path {
            let joined = checkout_path.join(relative);
            if joined.exists() {
                Ok(joined)
            } else {
                Err(GitResolutionError::MissingSubdirectory {
                    repo_path: checkout_path.to_owned(),
                    subdirectory: relative.clone(),
                })
            }
        } else {
            Ok(checkout_path.to_owned())
        }
    }
...
impl Resolver for GitResolver {
        ...
        H: ResolutionHandler<Self>,
    {
        ...
        if matches!(status, CheckoutStatus::Existing) {
            self.diagnostics
                .push(GitResolutionDiagnostic::ReusedCheckout(
                    checkout_path.clone(),
                ));
        }

        let ingot_path = Self::ingot_path(&checkout_path, description)?;
        let ingot_url = Url::from_directory_path(ingot_path.as_std_path()).map_err(|_| {
            GitResolutionError::CheckoutUrlConversion {
...
impl fmt::Display for GitResolutionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GitResolutionError::CheckoutUrlConversion { path } => {
                write!(f, "Failed to convert checkout path '{}' into a URL", path)
            }
...
impl std::error::Error for GitResolutionError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            GitResolutionError::CheckoutUrlConversion { .. } => None,
