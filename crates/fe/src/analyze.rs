...
fn write_report(out: &mut String, a: &SourceAnalysis, path: &str, gas_focus: bool) {
    ...
    writeln!(
        out,
        "Functions:  {} ({} unique structures, {:.1}% monomorphization)",
    ...
    if !dedup.entries.is_empty() {
        ...
        for e in dedup.entries.iter().take(limit) {
            ...
            writeln!(
                out,
                "{:<32} {:>6} {:>8} {:>8}",
                name, e.copies, e.stmts_per_copy, e.wasted
            )
            .unwrap();
        }
        writeln!(
            out,
            "\nTotal wasted: {} stmts ({:.1}% of bytecode)",
    ...
    if gas_focus {
        ...
        let biz_stmts = cats
            .iter()
            .find(|c| c.name == "Business")
            .map(|c| c.stmts)
            .unwrap_or(0);
        if abi_stmts > biz_stmts {
            writeln!(
                out,
                "Note: ABI encoding ({abi_stmts} stmts) exceeds business logic ({biz_stmts} stmts)."
            )
            .unwrap();
            writeln!(
                out,
                "Deduplication of ABI helpers is the highest-impact gas optimization."
            )
            .unwrap();
        }
    }
