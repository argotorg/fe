...
fn local_param_boundaries() {
    ...
    let key_start = SemanticQuery::at_cursor(&db, top, start_y).symbol_key()
    ...
    let key_last = SemanticQuery::at_cursor(&db, top, last_y).symbol_key()
    ...
    let key_param = SemanticQuery::at_cursor(&db, top, x_use).symbol_key()
        .expect("symbol for param x usage");
    // Def span should match a param header in the function
    let (_tm, def_span) = SemanticQuery::definition_for_symbol(&db, key_param).expect("def for param");
...
fn shadowing_param_by_local() {
    ...
    let key_use = SemanticQuery::at_cursor(&db, top, use_x).symbol_key()
    ...
    let param_key = SemanticQuery::at_cursor(&db, top, param_pos).symbol_key().expect("param key");
