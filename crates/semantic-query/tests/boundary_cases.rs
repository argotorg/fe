...
fn local_param_boundaries() {
    ...
    let key_start = SemanticQuery::at_cursor(&db, top, start_y)
        .symbol_key()
    ...
    let key_last = SemanticQuery::at_cursor(&db, top, last_y)
        .symbol_key()
        .expect("symbol at last char of y");
    assert_eq!(
        key_start, key_last,
        "identity should be stable across y span"
    );

    // Immediately after local 'y' (half-open end): should not select
    let after_y = last_y + parser::TextSize::from(1u32);

    let symbol_after = SemanticQuery::at_cursor(&db, top, after_y).symbol_key();
    ...
    let key_param = SemanticQuery::at_cursor(&db, top, x_use)
        .symbol_key()
        .expect("symbol for param x usage");
    // Def span should match a param header in the function
    let (_tm, def_span) =
        SemanticQuery::definition_for_symbol(&db, key_param).expect("def for param");
    let def_res = def_span.resolve(&db).expect("resolve def span");
    let name_text = &content.as_str()
        [(Into::<usize>::into(def_res.range.start()))..(Into::<usize>::into(def_res.range.end()))];
...
fn shadowing_param_by_local() {
    ...
    let key_use = SemanticQuery::at_cursor(&db, top, use_x)
        .symbol_key()
        .expect("symbol at x usage");

    // Def for resolved key should be the local 'x' binding
    let (_tm, def_span) = SemanticQuery::definition_for_symbol(&db, key_use).expect("def for x");
    let def_res = def_span.resolve(&db).expect("resolve def");
    let def_text = &content.as_str()
        [(Into::<usize>::into(def_res.range.start()))..(Into::<usize>::into(def_res.range.end()))];
    ...
    let param_key = SemanticQuery::at_cursor(&db, top, param_pos)
        .symbol_key()
        .expect("param key");
