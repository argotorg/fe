...
fn symbol_label<'db>(db: &'db dyn SpannedHirDb, adb: &'db dyn HirAnalysisDb, key: &fe_semantic_query::SymbolKey<'db>) -> String {
    ...
    match key {
        ...
        SymbolKey::Method(fd) => {
            ...
            if path.is_empty() { format!("method {}", name) } else { format!("{}::{}", path, name) }
        }
        SymbolKey::FuncParam(item, idx) => {
            let path = hir::hir_def::scope_graph::ScopeId::from_item(*item).pretty_path(db).unwrap_or_default();
...
fn symbol_keys_snapshot(fx: Fixture<&str>) {
    ...
    let file = db
        .workspace()
        .touch(&mut db, Url::from_file_path(fx.path()).unwrap(), Some(fx.content().to_string()));
    ...
    for (label, key) in entries {
        ...
        if let Some(d) = def_opt.as_ref() { if !files.contains(&d.file) { files.push(d.file); } }
        // Stable order by file URL path
        files.sort_by_key(|f| f.url(&db).map(|u| u.path().to_string()).unwrap_or_default());

        for f in files {
            let content = f.text(&db);
            let name = f.url(&db).and_then(|u| u.path_segments().and_then(|mut s| s.next_back()).map(|s| s.to_string())).unwrap_or_else(|| "<file>".into());
            ...
            if let Some(def) = def_opt.as_ref().filter(|d| d.file == f) {
                ...
                defs_same.push((s..e, format!("defined here @ {}:{} ({} refs)", l, c, total_refs)));
            }
