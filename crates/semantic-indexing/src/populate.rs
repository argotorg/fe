...
use crate::index::{IndexLocation, IndexReference, IndexSymbol, SemanticIndex};
...
pub fn populate(
    ...
    semantic_index.upsert_symbols(db, all_symbols);
    semantic_index.upsert_references(db, all_references);
}

/// Incrementally update the SemanticIndex for a single changed file.
pub fn update_for_file(
    db: &mut driver::DriverDataBase,
    semantic_index: SemanticIndex,
    file_url: &url::Url,
) {
...
fn item_symbol<'db>(
    ...
    let display_name = pretty_path
        ...
        .to_string();
    Some((::scip::symbol::format_symbol(symbol), display_name))
}

/// Index a single top-level module.
pub fn index_module<'db>(
    ...
    for item in scope_graph.items_dfs(db) {
        ...
        let Some((ref symbol_str, ref display_name)) = maybe_symbol else {
            index_unnamed_generic_params(
                db, item, ctx, &doc_url, file_relative_paths,
                &mut symbols, &mut refs_by_symbol, &mut line_index_cache,
            );
            continue;
        };
        ...
        let def_location = item
            ...
            .unwrap_or_else(|| IndexLocation {
                relative_path: file_relative_paths.get(&doc_url).cloned().unwrap_or_default(),
        ...
        for indexed_ref in ctx.ref_index.references_to(&scope) {
            if let Some(resolved) = indexed_ref.span.resolve(db) {
                if let Some(loc) = span_to_location(&resolved, db, file_relative_paths, &mut line_index_cache) {
                    refs_by_symbol.entry(symbol_str.clone()).or_default().push(IndexReference {
                        symbol: symbol_str.clone(),
                        location: loc,
                        role: 0,
                    });
        ...
        if !matches!(item, ItemKind::Mod(_) | ItemKind::TopMod(_)) {
            ...
            for child in sym_view.children(db) {
                ...
                let Some(child_name) = child.name(db) else { continue };
                ...
                if let Some(name_span) = child.name_span(db) {
                    if let Some(loc) = span_to_location(&name_span, db, file_relative_paths, &mut line_index_cache) {
                        symbols.push((
                            child_symbol.clone(),
                            IndexSymbol {
                ...
                for indexed_ref in ctx.ref_index.references_to(&child_scope) {
                    if let Some(resolved) = indexed_ref.span.resolve(db) {
                        if let Some(loc) = span_to_location(&resolved, db, file_relative_paths, &mut line_index_cache) {
                            refs_by_symbol.entry(child_symbol.clone()).or_default().push(IndexReference {
                                symbol: child_symbol.clone(),
                                location: loc,
                                role: 0,
                            });
                ...
                index_generic_params(
                    db, &child_view, &child_symbol, ctx, &doc_url,
                    file_relative_paths, &mut symbols, &mut refs_by_symbol, &mut line_index_cache,
                );
            }
        }
        ...
        let sym_view = SymbolView::from_item(item);
        index_generic_params(
            db, &sym_view, symbol_str, ctx, &doc_url,
            file_relative_paths, &mut symbols, &mut refs_by_symbol, &mut line_index_cache,
        );
    }
    ...
    Some(ModuleIndexResult {
        symbols,
        references: refs_by_symbol.into_iter().collect(),
    })
}
...
fn index_generic_params<'db>(
    ...
    for gp_scope in sym_view.generic_params(db) {
        let Some(gp_name) = gp_scope.name(db) else { continue };
        ...
        if let Some(name_span) = gp_scope.name_span(db)
            ...
            && let Some(loc) = span_to_location(&resolved, db, file_relative_paths, line_index_cache)
        {
            symbols.push((
                gp_symbol.clone(),
                IndexSymbol {
        ...
        for indexed_ref in ctx.ref_index.references_to(&gp_scope) {
            if let Some(resolved) = indexed_ref.span.resolve(db) {
                if let Some(loc) = span_to_location(&resolved, db, file_relative_paths, line_index_cache) {
                    refs_by_symbol.entry(gp_symbol.clone()).or_default().push(IndexReference {
                        symbol: gp_symbol.clone(),
                        location: loc,
                        role: 0,
                    });
...
fn index_unnamed_generic_params<'db>(
    ...
    let impl_offset = item.span().resolve(db).map(|s| u32::from(s.range.start())).unwrap_or(0);
    let parent_symbol = format!("fe fe {} {} __impl_{} ", ctx.name, ctx.version, impl_offset);

    let sym_view = SymbolView::from_item(item);
    index_generic_params(
        db, &sym_view, &parent_symbol, ctx, doc_url,
        file_relative_paths, symbols, refs_by_symbol, line_index_cache,
    );
