...
impl MockLspClient {
    /// Spawn the real server with a doc_regenerate_fn and connect a mock client.
    /// Returns the client plus the regen invocation counter.
    pub async fn start_with_doc_regen() -> (Self, Arc<std::sync::atomic::AtomicU64>) {
        use crate::backend::DocRegenerateFn;
        use crate::server::{setup_service, spawn_backend};

        let regen_count = Arc::new(std::sync::atomic::AtomicU64::new(0));
        let regen_counter = regen_count.clone();

        // Simulate an expensive regen: count invocations and sleep briefly
        // to mimic the cost of full SCIP generation (~100ms minimum).
        let doc_regen_fn: DocRegenerateFn = Arc::new(move |_db| {
            regen_counter.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            std::thread::sleep(std::time::Duration::from_millis(50));
            ("{}".to_string(), None)
        });

        let (server_main, server_client_socket) = MainLoop::new_server(move |client| {
            let actor_ref = spawn_backend(
                client.clone(),
                "test-doc-reload-actor".to_string(),
                None,
                Some(doc_regen_fn.clone()),
                None,
                None,
            );
...
// ---------------------------------------------------------------------------
// Regression test for issue #1424: doc reload regen count under rapid edits.
//
// Reproduces the scenario where every keystroke triggered a full SCIP
// regeneration, burning 200%+ CPU. The debounce stream coalesces rapid
// DocReloadRequest events into a bounded number of DocReloadExecute events.
// ---------------------------------------------------------------------------
...
async fn doc_reload_debounces_rapid_edits() {
    ...
    // Simulate 20 rapid keystrokes (no settle between them)
    for i in 0..20 {
    ...
    // With 200ms debounce, 20 rapid edits should coalesce into at most a few
    ...
    assert!(
        count >= 1,
        "doc reload never fired — doc_regenerate_fn may not be wired up"
    );
